...
const int trig_pin    = D4; // D4 is trigger pin for Lidar
const int mon_pin     = D5; // D5 is monitor pin for Lidar
...
void loop() {
  ...
  while(1) {
    ...
    pulseWidth = pulseIn(D5, HIGH); // Count how long the pulse is high in microseconds
    ...
    x=x+(bufX[p]-bufX[q]);
    y=y+(bufY[p]-bufY[q]);
    incX=atan(radio_data[0]/radio_data[1]);
